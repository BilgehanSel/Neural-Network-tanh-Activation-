		...
		for (unsigned i = error.size() - 2; i != -1; i--) {
			for (unsigned j = 0; j != error[i].size(); j++) {
				...
				for (unsigned k = 0; k != error[i + 1].size(); k++) {
					sum += error[i + 1][k] * weights[i + 1][k][j];
				}
				error[i][j] = sum;
			}
		}
...
void timeLeft(unsigned *i, unsigned *iterations) {
	while (true) {
		...
		std::cout << "%" << double(*i) / double(*iterations) * 100.0 << std::endl;
		if (*i >= *iterations)
		break;
	}
}
...
int main() {
	...
	//////////////////////
	// Since training and test functions take std::vector<std::vector<std::vector<double>>> typedef int name;
	// You need to supply them with that type, if we continue from our example(torque, and the dataset.txt). We have put the relevant data into these structures.
	...
	// Shuffling data since it's not nicely distributed
	std::random_shuffle(train_data.begin(), train_data.end());
	std::random_shuffle(train_data.begin(), train_data.end());
	std::random_shuffle(train_data.begin(), train_data.end());

	// chosing the topology of the neural network, (input size, layers in between and the output layer);
	std::vector<unsigned> topology = { train_data[0][0].size(), 4, 8, 16, 8, 4, train_data[0][1].size() };
	NeuralNetwork nn(topology);

	unsigned iterations = 100; // Our total iterations through the train_data vector
	unsigned i = 0; // our current iteration count
	std::thread t1(timeLeft, &i, &iterations); // Launching a new thread to see how far the training went
	t1.detach();
